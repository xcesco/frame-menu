...
(function ($, window, document, undefined) {
    ...
    function rowWorkSheetListToObject(cell) {
        var returner = {};

        returner['id'] = cell["id"]["$t"].split('/').pop();
        returner['title'] = cell["title"]["$t"];
    ...
    $.googleSheetToJSON = function googleSheetToJSON(id, worksheet) {
        ...
        $.getJSON(url)
            .done(function (data) {
                ...
                if (!data.feed) throw new Error('Risulta impossibile leggere lo spreadsheet per l\'url ' + url);
                if (!data.feed.entry) throw new Error('Google spreadsheet sembra esssere vuota per l\'url ' + url);
    ...
    $.googleWorkSheetListToJSON = function googleWorkSheetListToJSON(id) {
        ...
        $.getJSON(url)
            .done(function (data) {
                ...
                if (!data.feed) throw new Error('Risulta impossibile leggere lo spreadsheet per l\'url ' + url);
                if (!data.feed.entry) throw new Error('Google spreadsheet sembra esssere vuota per l\'url ' + url);
                deferred.resolve(data.feed.entry.map(rowWorkSheetListToObject));
            })
            .fail(deferred.reject);
